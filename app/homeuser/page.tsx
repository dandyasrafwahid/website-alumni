  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-[#1E3A8A] font-bold animate-pulse">Memuat Data...</div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 items-start gap-12">
            ...
            <section className="text-white pt-4 lg:pt-12">
              ...
              <h1 className="text-4xl lg:text-6xl font-extrabold uppercase tracking-wide leading-tight">
                {user?.name ? (
                    <>
                        <span className="block">{user.name.split(" ").slice(0, -1).join(" ")}</span>
                        <span className="text-yellow-400">{user.name.split(" ").slice(-1)}</span>
                    </>
                ) : (
                    "ALUMNI!"
                )}
              ...
              <p className="mt-6 text-base lg:text-lg text-gray-200 max-w-xl leading-relaxed">
                ...
                dengan almamater, dan menjalin jaringan yang kuat.
                Bersama-sama, mari kita terus menginspirasi.
              </p>
              ...
              <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg">
                <Link href="/alumni">
                    <div className="bg-white/10 hover:bg-white/20 border border-white/20 p-4 rounded-lg cursor-pointer transition-all group">
                        <h3 className="font-bold text-white group-hover:text-yellow-300">Cari Alumni →</h3>
                        <p className="text-xs text-gray-300 mt-1">Temukan teman seangkatan.</p>
                    </div>
                </Link>
                <Link href="/jobs">
                    <div className="bg-white/10 hover:bg-white/20 border border-white/20 p-4 rounded-lg cursor-pointer transition-all group">
                        <h3 className="font-bold text-white group-hover:text-yellow-300">Lowongan Kerja →</h3>
                        <p className="text-xs text-gray-300 mt-1">Peluang karir terbaru.</p>
                    </div>
                </Link>
              </div>
            </section>
